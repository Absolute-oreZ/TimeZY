				...
				<body class="bg-gray-100 w-full flex flex-1">
					<div class="flex flex-1">
						<div class="p-8 self-center w-full rounded shadow-md md:w-1/2 items-center justify-center">
							...
							<form action="/${currentUser.role.toString().toLowerCase()}/task/edit/${task.taskId}" method="post" class="space-y-4">
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
									<!-- Task Name -->
									<div>
										<label for="taskName"
											class="block text-sm font-medium text-gray-700">Task:</label> <input
											type="text" id="taskName" name="taskName" value="${task.taskName}" required
									...
									<div>
										<label for="cleaner" class="block text-sm font-medium text-gray-700">Responsible
											...
											class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
											<option selected hidden value="${task.cleaner.userId}">${task.cleaner.name}
											</option>
									...
									<div>
										...
										<select id="taskCategory" name="taskCategory"
											class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
											<option selected hidden value="${task.taskCategory}">
									...
									<div>
										...
										<select id="taskPriority" name="taskPriority" required
											class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
											<option value="${task.taskPriority}" hidden selected>
												${task.taskPriority.toString()}</option>
									...
									<div class="col-span-2">
										...
										<textarea id="taskDescription" name="taskDescription"
											...
											class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">${task.taskDescription}</textarea>
									</div>
									...
									<div class="col-span-2">
										...
										<select id="location" name="location" required
											class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
											<option value="${task.location.locationId}" hidden selected >${task.location.locationName},${task.location.buildingName},${task.location.floorNumber}</option>
									...
									<div class="col-span-2">
										<label for="taskDeadline" class="block text-sm font-medium text-gray-700">Task
											Deadline</label> <input type="datetime-local" name="taskDeadline" value="${task.taskDeadline}"
						...
						<div class="md:flex-col p-8 w-1/2 m-2 items-center justify-center hidden md:flex">
							...
							<div class="text-slate-800 text-center py-4 px-6 font-montserrat font-bold text-3xl">
								<span class="text-7xl font-extrabold">"</span>Ambitions is merely a task away from reality
							</div>
						</div>
					</div>
				</body>
